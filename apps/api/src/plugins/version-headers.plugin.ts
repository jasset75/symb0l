  ...
  fastify.addHook("onRequest", async (request, reply) => {
    ...
    if (versionStatus === VersionStatus.SUNSETTED) {
      return reply.code(410).send({
        statusCode: 410,
        error: "Gone",
        message: `API version ${resolvedVersion} has been sunset and is no longer available. Please upgrade to version ${config.stable}.`,
        stableVersion: config.stable,
      });
    }

    // Always add these headers
    reply.header("X-Resolved-Version", resolvedVersion);
    reply.header("X-API-Stable-Version", config.stable);

    // Add deprecation headers if version is deprecated
    if (versionStatus === VersionStatus.DEPRECATED) {
      const deprecationInfo = config.apiVersions.deprecated[resolvedVersion];
      if (deprecationInfo) {
        ...
        reply.header(
          "Link",
          `</v${config.stable}${basePath}>; rel="successor-version"`,
        );
      }
    }
  });
